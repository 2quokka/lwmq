...
void lwmqtt_topic_check(const char *topic, bool *opt){
	if(topic == NULL)
		return;
...
lwmqtt_message_t* lwmqtt_create_msg(lwmqtt_qos_t qos, bool retained, uint8_t *payload, int verif, const char *username){
	
	lwmqtt_message_t *msg = malloc(sizeof(lwmqtt_message_t));
	memset(msg, 0, sizeof(lwmqtt_message_t));

	msg->qos = qos;
	msg->retained = retained;
	msg->payload_len = strlen((char *)payload) + (verif ? 32 : 0);
	...
	if(verif){
		...
		memset(msg->digest, 0, 50);
		hmac_sha256(username, strlen(username), payload, strlen((char*)payload), msg->digest, &(msg->digest_len));
		strcat((char *)(msg->payload), (char *)(msg->digest));
	}
...
static lwmqtt_err_t lwmqtt_cycle(lwmqtt_client_t *client, size_t *read, lwmqtt_packet_type_t *packet_type, char **id_list) {
...
static lwmqtt_err_t lwmqtt_cycle_until(lwmqtt_client_t *client, lwmqtt_packet_type_t *packet_type, size_t available, lwmqtt_packet_type_t needle, char **id_list) {
  // prepare counter
  size_t read = 0;
  ...
  do {
    // do one cycle
    lwmqtt_err_t err = lwmqtt_cycle(client, &read, packet_type, id_list);
    if (err != LWMQTT_SUCCESS) {
      return err;
    }
...
lwmqtt_err_t lwmqtt_yield(lwmqtt_client_t *client, size_t available, uint32_t timeout, char **id_list) {
  // set command timer
  ...
  lwmqtt_err_t err = lwmqtt_cycle_until(client, &packet_type, available, LWMQTT_NO_PACKET, id_list);
  if (err != LWMQTT_SUCCESS) {
    return err;
  }
...
lwmqtt_err_t lwmqtt_connect(lwmqtt_client_t *client, lwmqtt_options_t options, lwmqtt_will_t *will,
  ...
  *return_code = LWMQTT_UNKNOWN_RETURN_CODE;
  ...
  lwmqtt_packet_type_t packet_type = LWMQTT_NO_PACKET;
  err = lwmqtt_cycle_until(client, &packet_type, 0, LWMQTT_CONNACK_PACKET, NULL);
  if (err != LWMQTT_SUCCESS) {
    return err;
...
lwmqtt_err_t lwmqtt_subscribe(lwmqtt_client_t *client, int count, lwmqtt_string_t *topic_filter, lwmqtt_qos_t *qos,
  ...
  lwmqtt_packet_type_t packet_type = LWMQTT_NO_PACKET;
  err = lwmqtt_cycle_until(client, &packet_type, 0, LWMQTT_SUBACK_PACKET, NULL);
  if (err != LWMQTT_SUCCESS) {
    return err;
...
lwmqtt_err_t lwmqtt_unsubscribe(lwmqtt_client_t *client, int count, lwmqtt_string_t *topic_filter, uint32_t timeout) {
  ...
  lwmqtt_packet_type_t packet_type = LWMQTT_NO_PACKET;
  err = lwmqtt_cycle_until(client, &packet_type, 0, LWMQTT_UNSUBACK_PACKET, NULL);
  if (err != LWMQTT_SUCCESS) {
    return err;
...
lwmqtt_err_t lwmqtt_publish(lwmqtt_client_t *client, lwmqtt_string_t topic, lwmqtt_message_t message,
  ...
  // wait for ack packet
  lwmqtt_packet_type_t packet_type = LWMQTT_NO_PACKET;
  err = lwmqtt_cycle_until(client, &packet_type, 0, ack_type, NULL);
  if (err != LWMQTT_SUCCESS) {
    return err;
